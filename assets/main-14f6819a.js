...
class Perfil {
  ...
  static async getRol(id) {
    await supabase.from("perfiles").select("rol").eq("user_id", id);
    return Perfil.rol;
  }
}
class User {
...
const menuSuperior = {
  ...
  script: (rolUsuario) => {
    const items = {
      ...
      <li class="nav-item">
        <a class="nav-link text-black" href="#/recursos">Recursos</a>
      </li>
      <li><hr /></li>
...
const footer = {
  template: `<nav class="navbar navbar-expand-sm bg-white fixed-bottom text-center justify-content-center text-black">
        <p>Pau Diaz Calabuig</p>
    </nav>`
};
const styles = "";
var top = "top";
